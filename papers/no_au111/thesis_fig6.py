...
tdpt_pes_args = {'marker' : 'D', 'linestyle' : '-','color' : 'orange', 'label' : r'MDEF(ODF)$^{\dag}$', 'alpha' : 1.0}
bomd_pes_args = {'marker' : 'v', 'linestyle' : '-','color' : 'cyan', 'label' : r'BOMD$^{\dag}$', 'alpha' : 1.0}
annotate_args = {'xy' : (0.94,0.9), 'xycoords' : 'axes fraction'}
...
ax[0].set_xlabel('$E_{\mathrm{i}}$ / eV')#,color='white')
...
ax[0].legend(handles=handles,labels=labels,ncol=2,handletextpad=0.1,columnspacing=0.3,fancybox=True,framealpha=0,handlelength=1.5,bbox_to_anchor=(0.5, 1.2), loc='center')
...
fig.savefig('thesis_fig6.pdf',transparent=True,bbox_inches='tight')
