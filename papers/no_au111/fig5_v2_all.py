...
gs01 = gridspec.GridSpecFromSubplotSpec(2, 1, subplot_spec=gs0[:], hspace=0.4)
...
ax[1,0].annotate(r'(a)',ha="left", **annotate_args)
ax[1,1].annotate(r'(b)',ha="left", **annotate_args)
indices=[]
for i,filename in enumerate(filenames):
    ...
    if mode in ['BOMD','LDFA','ODF','LDFA*4','ODF*4','ODF(rr)*4','ODF[RS]']:
        with open('figs9.txt','a+') as f:
...
labels[0]=labels[0]+')'
plt.legend(handles=handles,labels=labels,ncol=2,handletextpad=0.15,columnspacing=0.6,fancybox=True,framealpha=0,handlelength=2,bbox_to_anchor=(0.5, 2.65), loc='center')
#plt.tight_layout()
#plt.subplots_adjust(hspace=1.2)
#plt.gcf().subplots_adjust(right=0.01)
fig.savefig('fig5_all.pdf',transparent=True,bbox_inches='tight')
fig.savefig('fig5_all.tiff',transparent=True,bbox_inches='tight',dpi=600)
fig.savefig('fig5_all.eps',transparent=True,bbox_inches='tight')
