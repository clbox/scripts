...
for i in range(dimension):
    for j in range(dimension):
        if j >= i:
            ...
            ax[i, j].plot(e_axis*hartree,real,'o',markersize = 2,mfc='none',color = 'purple')
            ax[i, j].plot(x*hartree,ys,linewidth = 1, color = 'orange')


            ax[i, j].text(0.35, 0.7, str((i+1, j+1)), transform=ax[i, j].transAxes,
                          fontsize=11,alpha=0.2)
            ax[i, j].set_xlim(0,max_e)
...
for j in range(dimension):
    for i in range(dimension):
        if j <= i:
            c += 1
            ax[i, j].plot(e_axis*hartree,imag*10,markersize = 2, mfc='none',color = 'cyan')
...
fig3.text(0.02, 0.5, r'Imaginary coupling element $\times$ 10 / $ps^{-1}$', va='center', rotation='vertical')
fig3.text(0.92, 0.5, r'Real coupling element / ps$^{-1}$', va='center', rotation='vertical')
fig3.savefig('tensor_spectrum_both_inverted_close.pdf')
print("Generated plot 'tensor_spectrum_both_inverted_close.pdf'")
