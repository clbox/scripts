...
for j in range(dimension):
    for i in range(dimension):
        if j <= i:
            c += 1
            ax[i, j].plot(e_axis*hartree,imag*10/ps,markersize = 2, mfc='none',color = 'cyan')
            ax[i, j].set_xlim(0,max_e)
        if j < i:
            ax[i, j].text(0.35, 0.7, str((i+1, j+1)), transform=ax[i, j].transAxes,
                          fontsize=11,alpha=0.2)
...
fig3.savefig('tensor_spectrum_both_inverted'+str(nknots)+'.pdf')
print('Generated plot tensor_spectrum_both_inverted '+str(nknots)+'.pdf')
