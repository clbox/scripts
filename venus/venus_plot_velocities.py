...
labels = [r'$rr$',r'$\theta\theta$',r'$\phi\phi$',r'$XX$',r'$YY$',r'$ZZ$']
line_args = {'linewidth' : 1.0}
colours = ['red','dodgerblue','black','magenta','orange','green']
...
ax.set_ylim(bottom=0.0,top=0.001)
...
ax.set_xlabel('Time / ps',fontsize=9.5,fontname=font)#,color='white')
ax.set_ylabel(r'$\mid\dot{R}\mid ^{2}$ / $\mathrm{ \AA{}}^{2}$ fs$^{-2}$',fontsize=9.5,fontname=font)#,color='white')
fig.savefig('velocities.pdf',transparent=True)#,bbox_inches='tight')


fig.legend(ncol=6,fontsize=9.5,fancybox=True,framealpha=0)
fig.set_figwidth(7)
ax.remove()
