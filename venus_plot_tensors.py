...
for i in range(ndim):
    ...
    if i <=2:
        ...
        linestyle='-'
    ax.plot(time_axis,projected_tensors[:,i,i],label=labels[i],linestyle=linestyle,**line_args,color=colours[i])
...
fig.savefig('tensors.pdf',transparent=True)#,bbox_inches='tight')
